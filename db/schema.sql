...
CREATE TABLE task (id INTEGER PRIMARY KEY AUTOINCREMENT,idAbonnee INTEGER, title VARCHAR(255), list_id INTEGER,oldPrice INTEGER,newPrice INTEGER,startDate DATE,due DATE, reminder DATE, done INTEGER, note BLOB);
CREATE TABLE list (id INTEGER PRIMARY KEY AUTOINCREMENT,idAbonnee INTEGER, name VARCHAR(50), leaf INTEGER, lft INTEGER, rgt INTEGER);
